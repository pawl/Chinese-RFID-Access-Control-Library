...
add_badge_resp_2 = (
    b'#!\xee\xb1)\x00\x00\x00\x9dtV\x07\x00\x00\x00\x00\x01\x05\x02\x00'
)

remove_user_resp = (
    b'#!\xa9\x86"\x00\x00\x00\x9dtV\x07\x00\x00\x00\x00\x01\x05\x02\x00'
)
...
class TestRFIDClient(unittest.TestCase):
    ...
    @patch(
        "rfid.RFIDClient.connect", return_value=mock_socket([add_badge_resp_2, add_badge_resp_1])
    )
    @patch("rfid.RFIDClient.check_valid_ipv4_address")
